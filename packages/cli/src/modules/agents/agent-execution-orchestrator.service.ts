...
function getDelegatedChildCheckpoints(
	...
	for (const pendingToolCall of Object.values(checkpoint.pendingToolCalls)) {
		...
		const ownerAgentId = getDelegatedChildOwnerAgentId(childCheckpoint, parentAgentId);
...
function getDelegatedChildOwnerAgentId(
	checkpoint: NonNullable<ReturnType<typeof parseDelegateSubAgentContinuation>>,
	parentAgentId: string,
): string | undefined {
	if (checkpoint.resumeContext === undefined) {
		return checkpoint.subAgentId === INLINE_SUB_AGENT_ID ? parentAgentId : undefined;
	}
	if (
		!isRecord(checkpoint.resumeContext) ||
		typeof checkpoint.resumeContext.agentId !== 'string' ||
		checkpoint.resumeContext.agentId.length === 0 ||
		(checkpoint.resumeContext.versionId !== undefined &&
			(typeof checkpoint.resumeContext.versionId !== 'string' ||
				checkpoint.resumeContext.versionId.length === 0))
	) {
		return undefined;
	}

	const expectedOwnerAgentId =
		checkpoint.subAgentId === INLINE_SUB_AGENT_ID ? parentAgentId : checkpoint.subAgentId;
	return checkpoint.resumeContext.agentId === expectedOwnerAgentId
